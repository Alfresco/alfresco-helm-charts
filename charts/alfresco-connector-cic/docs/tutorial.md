...
This guide walks you through connecting a CIC chart to a previously deployed repository
instance, with a focus on configuring the live ingester. It includes setting up
the CIC Helm chart and ensuring the repository instance is correctly configured
for the repository-side HXI integration properties it still consumes.
...
1. **Prepare your [file](cic.yml)** with all required connection settings for
   CIC, ActiveMQ, and the repository. This file is passed into the Helm chart
   and should include the CIC client details, environment key, URLs, and the
...
helm repo update
helm install cic alfresco-helm-charts/alfresco-connector-cic --values charts/alfresco-connector-cic/docs/cic.yml
```

3. **Review the output** from the Helm chart installation. You will see a list
   of generated repository properties that need to be passed to the repository instance.
   These remain `hxi.*` because they are consumed by the repository-side integration:
