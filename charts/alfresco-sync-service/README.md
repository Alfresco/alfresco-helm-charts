...
|-----|------|---------|-------------|
| activemq | object | `{"adminUser":{"password":null,"user":null},"enabled":false,"nameOverride":"activemq","services":{"broker":{"ports":{"external":{"openwire":61616}}}}}` | messageBroker object allow to pass ActiveMQ connection details. url: provides URI formatted string, see: https://activemq.apache.org/failover-transport-reference user: username to authenticate as. password: credential to use to authenticate to the broker. |
| activemq.adminUser.password | string | `nil` | Password to use to set as the connection user for ActiveMQ |
| activemq.adminUser.user | string | `nil` | User to use to set as the connection user for ActiveMQ |
| activemq.nameOverride | string | `"activemq"` | Activemq chart options (see [Alfresco ActiveMQ chart documentation](https://github.com/Alfresco/alfresco-helm-charts/tree/main/charts/activemq)) |
...
| messageBroker.existingSecretName | string | `nil` | An existing kubernetes secret with MQ info (prefered over using values) |
| messageBroker.password | string | `"admin"` |  |
| messageBroker.url | string | `nil` |  |
| messageBroker.user | string | `"alfresco"` |  |
| nodeSelector | object | `{}` |  |
